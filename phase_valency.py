...
parser.add_option("-v", "--valences", dest="domains", default=4, type=int, help="number of domains in a molecule (valency) (default=4)")
...
for iteration in range(updates+1):
            ...
            for d in range(len(data[iteration][j, 14:])):
                if int(data[iteration][j, (14 + d):(15 + d)]) == 0:  # location of a free domain is found
                    ...
                    for l in range(len(temp_counterparts)):
                        ...
                        association_prob = round(((0.6 * 0.9 * 20) / (20 + total_in_cube * domains)) * 1000)  # ensure that there are 4 significant digits
    ...
    for j in range(data[0].shape[0]):
        if data[iteration][j, 13:14] == 0:                 # molecules with no partner will 100% diffuse
            ...
            move_or_not = random.choices([1, 2], weights=[diffuse_probability, (1000-(diffuse_probability))], k=1)
            if move_or_not == [1]:
                temp_xyz = np.random.choice([1, 2, 3])
                temp_minusplus = np.random.choice([-1, 1])
            elif move_or_not == [2]:
